...
class EmailService {
  ...
  async sendVerificationEmail(email, token) {
    const verificationUrl = `${process.env.CLIENT_URL || 'http://localhost:5000'}/api/users/verify/${token}`;

    const mailOptions = {
      from: `"TaskPulse Support" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Verify Your TaskPulse Account',
      html: `
  ...
  async sendTaskSummaryEmail(email, name, summary) {
    ...
    const mailOptions = {
      from: `"TaskPulse Daily Summary" <${process.env.EMAIL_USER}>`,
      to: email,
